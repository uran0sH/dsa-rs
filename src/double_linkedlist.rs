...
impl<T> LinkedList<T> {
    ...
    fn _get_by_idx(&self, idx: usize) -> Result<Option<NonNull<Node<T>>>, Box<dyn Error>> {
        let len = self.length;
    ...
    pub fn insert_by_idx(&mut self, idx: usize, data: T) -> Result<(), Box<dyn Error>> {
        ...
        unsafe {
            ...
            let before_node = self._get_by_idx(idx - 1)?;
    ...
    pub fn remove_by_idx(&mut self, idx: usize) -> Result<T, Box<dyn Error>> {
        ...
        let cur = self._get_by_idx(idx)?.unwrap();
